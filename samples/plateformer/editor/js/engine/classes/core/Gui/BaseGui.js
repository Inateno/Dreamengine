...
function( CONFIG, COLORS, Vector2, Sizes )
{
  function BaseGui( param )
  {
    ...
    this.gui = null;
    this.id = param.id || param.name || Date.now();
    ...
    */
    ...
    {
      this.isDisable = false;
      ctx.clearRect( this.position.x, this.position.y, this.sizes.width * this.sizes.scaleX, this.sizes.height * this.sizes.scaleY );
      this.render( ctx );
      ...
      if ( CONFIG.DEBUG )
      {
        ...
        ctx.strokeRect( this.position.x + 1, this.position.y + 1, this.sizes.width * this.sizes.scaleX - 2, this.sizes.height * this.sizes.scaleY - 2 );
      }
    }
